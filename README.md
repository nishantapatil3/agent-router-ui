...
| **Settings** | Theme (light / dark / follow OS), gateway and admin base URLs with a connectivity probe, Playground defaults and the Usage refresh interval. |
...
`#models`, `#usage`, `#tools` and `#settings` are linkable.
...
`.github/workflows/docker.yml` publishes images to GitHub Container Registry on
every push to `main` (`latest` plus the commit sha) and on `v*` tags (`1.2.3`,
`1.2`); pull requests build the image without publishing. Pull one with
...
One difference from `npm run dev`: the proxy targets shown on the Settings page
are baked in at build time (`--build-arg AIGW_URL=... --build-arg
AIGW_ADMIN_URL=...` to match); they are display-only and do not affect routing.
...
- **Data** — Usage auto-refresh interval (0 disables it).

## Notes and limits

- **There is no request log.** A browser cannot read the gateway's access log
  and aigw exposes no API for one — its admin server serves only `/health` and
  `/metrics` — so per-request history would have to come from a log store the
  stack does not run. Use the Compose project's `logs/aigw/envoy-runs/0/stdout.log`
  (or `docker compose logs aigw` with `AIGW_DEBUG=true`) for that, and Grafana
